...
const ChatForm = (props) => {
   ...
   const {call, callAccepted, callEnded, stream, name} = useSelector((state) => state.video);
   ...
   useEffect(() => {
      (async () => {
         const res = await fetch(`http://localhost:5000/conversation/messages/${conversation._id}`, {
            credentials: "include"
         });
   ...
   const onClickHandler = async (e) => {
      ...
      if (message) {
         ...
         socket.emit("sendMessage", {userId: user._id, message: message, conversationId: conversation._id});
         setMessage("");
         await postData({newMessage: oldMes}, `http://localhost:5000/conversation/new-message/?conversationId=${conversation._id}&userId=${user._id}`);
      }
   }
   ...
   const clickVideoCall = async (e) => {
      ...
      socket.emit("make-connection-call", {conversationId: conversation._id, caller: user}, (callee) => {
         dispatch(videoActions.setCall({call: {isRecievedCall: false, caller: user, callee: callee, signal: null}}));
      });
      setTimeout(() => {
   ...
   return (
      <ChatFormContainer>
         ...
         <Input clickHandler={onClickHandler} message={message} changeHandler={onChangeHandler} />
         {
