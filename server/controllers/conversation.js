...
exports.getConversations = async (req, res, next) => {
   try {
      ...
      let conversations = [];
      if (userId !== "error") {
         conversations = await Conversation.find({"members": {$in: [userId]}}).populate({path: "members"});
      }
...
exports.postNewConversation = async (req, res, next) => {
   try {
      ...
      const existedConversation = await Conversation.findOne({$and: [{members: friend._id}, {members: userId}]});
