...
const uri = "mongodb+srv://kietcao:hokgadau123@cluster0.ps9wx.mongodb.net/chat-zoom?";
...
const store = new MongoDBStore({
   uri: uri, collection: "session-store"
});
app.use(session({
   secret: "my secret",
   resave: false,
...
io.on("connection", (socket) => {
   ...
   socket.on("join-video", async ({userId}) => {
      const conversations = await Conversation.find({"members": {$in: [userId]}}).populate({path: "members"});
      socket.join(conversations.map((conversation) => conversation._id.toString()));
   });
   socket.on("join-message", async ({conversationId}) => {
   ...
   socket.on("make-connection-call", async ({conversationId, caller}, cb) => {
      const conversation = await Conversation.findById(conversationId).populate({path: "members"});
      const callee = conversation.members.find((user) => user._id.toString() !== caller._id);
      cb(callee);

      // sending to all clients except sender
      socket.broadcast.to(conversationId).emit("make-connection-call", {conversationId, conversation, caller, callee});
   });
